...
export class DeviceService {
 ...
 static async updateTrustStatus(
  ...
  console.log('[API] Verifying trust update signature:', {
   payload: payloadToVerify,
   signatureLength: signature.length,
   publicKeyPreview: actingDevice.publicKey.slice(0, 15) + '...',
  });
  ...
  if (!isValid) {
   console.warn('[API] Signature verification failed for payload:', payloadToVerify);
   throw new Error("Invalid device signature. Action denied.");
  }
