    ...
    const runCleanup = async () => {
      ...
      try {
        ...
        await db.delete(vault)
          .where(lt(vault.deletedAt, sevenDaysAgo));
        
        // 2. Delete cancelled notifications
        await db.delete(notificationSchedule)
          .where(eq(notificationSchedule.status, 'cancelled'));
        ...
        await db.delete(vault)
          .where(
            and(
              eq(vault.isCorrupted, true),
              lt(vault.corruptedAt, threeDaysAgo)
            )
          );
