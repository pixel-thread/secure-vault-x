...
export const DeviceStoreManager = {
  ...
  setDkek: async (value: string) =>
    await SecureStore.setItemAsync(KEYS.DKEK, value),
  removeDkek: async () => await SecureStore.deleteItemAsync(KEYS.DKEK),

  // MEK
  getMek: async () => await SecureStore.getItemAsync(KEYS.MEK),
  setMek: async (value: string) =>
    await SecureStore.setItemAsync(KEYS.MEK, value),
  ...
  setDeviceId: async (value: string) =>
    await SecureStore.setItemAsync(KEYS.DEVICE_ID, value),
  removeDeviceId: async () => await SecureStore.deleteItemAsync(KEYS.DEVICE_ID),

  // Reserve Device ID
  getDeviceIdReserve: async () =>
    await SecureStore.getItemAsync(KEYS.DEVICE_ID_RESERVE),
  setDeviceIdReserve: async (value: string) =>
    await SecureStore.setItemAsync(KEYS.DEVICE_ID_RESERVE, value),
  removeDeviceIdReserve: async () =>
    await SecureStore.deleteItemAsync(KEYS.DEVICE_ID_RESERVE),

  // Private Key
  getDevicePrivateKey: async () =>
    await SecureStore.getItemAsync(KEYS.DEVICE_PRIVATE_KEY),
  setDevicePrivateKey: async (value: string) =>
    await SecureStore.setItemAsync(KEYS.DEVICE_PRIVATE_KEY, value),
  ...
  setBiometricEnabled: async (value: boolean) =>
    await SecureStore.setItemAsync(
      "SV_BIOMETRIC_ENABLED",
      value ? "true" : "false",
    ),
  removeBiometricEnabled: async () =>
    await SecureStore.deleteItemAsync("SV_BIOMETRIC_ENABLED"),
